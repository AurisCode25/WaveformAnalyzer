...
static void stats(const CSV_Inputs *memory, int n, int field_offset, double *min_output, double *max_output, double *avg_output) {
    ...
    double N_MIN = output, N_MAX = output, N_SUM = 0.0;
...
void write_output(const char *filename, const Results *results, int N_phases, const CSV_Inputs *memory, int n) {
    ...
    fprintf(file, "Dataset: %d samples parsed at %.0f samples/s (%.2f s)\n", n, SAMPLE_RATE, (double)n / SAMPLE_RATE);
    ...
    for (int i = 0; i < N_phases; i++) {
        ...
        fprintf(file, "  Clipping: %d samples\n", r->Clipping);

        total_clipping += r->Clipping;
        if (!r->Compliance) {
            all_compliant = 0;
        }
    }
    ...
    for (int i = 0; i < N_phases; i++) {
        fprintf(file, "  Phase %c: %d samples clipped %s\n",
            results[i].Phase,
            results[i].Clipping,
            results[i].Clipping > 0 ? "[Clipping Detected]" : "[No Clipping]");
    }

    fprintf(file, "Total: %d samples clipped %s\n",
        total_clipping,
        total_clipping > 0 ? "[Clipping Detected]" : "[No Clipping]");
